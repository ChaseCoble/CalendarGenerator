// comments
//1. Yes, this is a more concise version of my implementation for finding out if the dates are valid and even included the reason.
//2. Yes, this calls the previous function since it is a very repeatable function
//3. Yes, this is similar to mine, however I did not know you could stack cases like that for a switch statement.
...
// post-condition: get the input of a date. If the date is NOT valid, will prompt the user to re-enter a valid date

void get_input(int& month, int& year);
// post-condition: get the input of a month. If the month is NOT valid, will prompt the user to re-enter a valid month

void get_input(int& year);
// post-condition: get the input of a year. If the date is NOT valid, will prompt the user to re-enter a valid year.
...
// post-condition: return if the specified month is valid, false otherwise
...
// Implementation of all functions above are given in this file                   
...
int daysInMonth(int month, int year)
{
	...
	//             if this implementation is the same as yours in Project 05.
...
{
	{
		...
		if (output.is_open()) {
			for (int i = 1; i <= 12; i++) // print out 12 month calendar
			{
				print_calendar(i, year, output); // print out ith month
				cout << endl;
			}
			output.close();
